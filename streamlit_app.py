...
from db.stats import (init_db, save_profile, get_profile, save_run, get_runs, get_stats,
                      ...
                      save_meetup, get_upcoming_meetups)
...
with col_h1:
    ...
    if streak >= 7:
        st.markdown(f"🔥🔥🔥 **Streak {streak} วัน** — ยอดเยี่ยมมาก!")
    elif streak >= 3:
        st.markdown(f"🔥🔥 **Streak {streak} วัน** — keep it up!")
    elif streak >= 1:
        st.markdown(f"🔥 **Streak {streak} วัน** — เริ่มต้นดี!")
with col_h2:
    st.write(f"👤 {profile.get('display_name', st.session_state.username)}")
...
with tab_plan:
    ...
    with col_map:
        ...
        if st.session_state.last_route and st.session_state.last_lat:
            ...
            folium.PolyLine(line, color=route_color, weight=5, opacity=0.85).add_to(m)
            ...
            st_folium(m, width=None, height=480, key="map_with_route")
            ...
            st_folium(base_map(lat, lon), width=None, height=480, key="map_default")
    ...
    if run_btn:
        ...
        st.session_state.last_places = get_nearby_places(lat, lon, radius_m=600)
    ...
    if nearby_places and st.session_state.last_route:
        ...
        for tab_n, group in zip(tabs_nearby, [food, drink, shop]):
            with tab_n:
                if group:
                    ...
                    st.caption("ไม่พบในรัศมี 600 เมตร")
...
4. นัดเจอที่จุดเริ่มต้น แล้ววิ่งด้วยกัน! 🏃‍♂️🏃‍♀️
            ...
            line = [[p[1], p[0]] for p in route.geometry]
            folium.PolyLine(line, color="#16a34a", weight=5, opacity=0.85,
                            tooltip=f"เส้นทางกลุ่ม {route.total_km} km").add_to(m)
            if line:
                lats = [p[0] for p in line]
                lons = [p[1] for p in line]
                m.fit_bounds([[min(lats), min(lons)], [max(lats), max(lons)]])
            st_folium(m, width=None, height=380, key="map_group")
