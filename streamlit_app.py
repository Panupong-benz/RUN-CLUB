...
from db.stats import (init_db, save_profile, get_profile, save_run, get_runs, get_stats,
                      ...
                      get_today_route_count, get_cached_route, save_route_cache,
                      get_token_usage_today)
from models.response import RouteResponse

init_db()

DAILY_ROUTE_LIMIT = int(st.secrets.get("DAILY_ROUTE_LIMIT") or 10)
...
with tab_plan:
    ...
    with col_form:
        ...
        with st.expander("⚙️ ตั้งค่าผู้วิ่ง"):
            ...
            tolerance_pct = st.slider(
                ...
                help=(
                    "ระยะทางจริงที่ยอมรับได้ เช่น ตั้ง 5 km + เผื่อ 15% = ยอมรับ 4.25–5.75 km\n\n"
                    "• **5%** — แม่นยำมาก (ใช้เวลานานกว่า)\n"
                    "• **15%** — สมดุลระหว่างความแม่นและความเร็ว ✅\n"
                    "• **30%** — เร็วที่สุด แต่ระยะอาจเบี่ยงได้มาก"
                ),
            )
            if target_km:
                _lo = round(target_km * (1 - tolerance_pct / 100), 2)
                _hi = round(target_km * (1 + tolerance_pct / 100), 2)
                st.caption(f"ช่วงที่ยอมรับ: **{_lo} – {_hi} km**")

        _today_count = get_today_route_count(st.session_state.username)
        _remaining = max(0, DAILY_ROUTE_LIMIT - _today_count)
        if _remaining == 0:
            st.error(f"⛔ วันนี้ใช้ครบ {DAILY_ROUTE_LIMIT} เส้นทางแล้ว — กลับมาพรุ่งนี้!")
        else:
            st.caption(f"วันนี้เหลือ **{_remaining}/{DAILY_ROUTE_LIMIT}** เส้นทาง")
        run_btn = st.button("🗺️ วางแผนเส้นทางวิ่ง", type="primary",
                            use_container_width=True, disabled=(_remaining == 0))
        _saved_count = len(get_saved_routes(st.session_state.username))
        if _saved_count > 0:
            st.caption(f"💡 มีเส้นทางโปรด {_saved_count} เส้น → แท็บ ❤️ Route โปรด โหลดได้ทันที ไม่ต้องรอ AI")
    ...
    if run_btn:
        ...
        # ── Check route cache first (saves 100% tokens if hit) ──
        cached_json = get_cached_route(lat, lon, final_km, night_mode)
        if cached_json:
            result = RouteResponse.model_validate_json(cached_json)
            st.session_state.last_route = result
            ...
            st.toast("⚡ โหลดจาก Cache — เร็วกว่าปกติ!")
            st.rerun()
        ...
        with st.spinner(f"⏳ AI กำลังวางแผนเส้นทาง {label}{final_km:.1f} km..."):
            try:
                result = plan_route(lat, lon, final_km, weight_kg, pace_val,
                                   night_mode=night_mode, tolerance_pct=float(tolerance_pct))
            except Exception as e:
                st.error(f"❌ {e}")
                st.stop()

        # Save to cache for next time
        save_route_cache(lat, lon, final_km, night_mode,
                         result.model_dump_json(), result.tokens_used)
        ...
        st.toast(f"✅ วางแผนสำเร็จ!{' 🌙' if night_mode else ''}")
        st.rerun()
...
with tab_profile:
    ...
    with col_stats:
        ...
        st.divider()
        st.subheader("⚡ เคล็ดลับประหยัดเวลารอ AI")
        _today_c = get_today_route_count(st.session_state.username)
        _usage = get_token_usage_today()
        cache_hits = max(0, _today_c - _usage["total_routes"])

        st.info(
            "**วิธีให้แอพตอบเร็วขึ้น:**\n\n"
            "❤️ **บันทึก Route โปรด** — โหลดเส้นทางเดิมได้ทันทีโดยไม่ต้องรอ AI\n\n"
            "⚡ **วางแผนซ้ำในพื้นที่เดิม** — ระบบจำเส้นทางไว้ 6 ชั่วโมง โหลดเร็วกว่าปกติ\n\n"
            "🎯 **เพิ่มค่าเผื่อระยะทาง** — ยิ่งเผื่อมาก AI ปรับน้อยรอบ ยิ่งเร็ว"
        )
        u1, u2 = st.columns(2)
        u1.metric("🗺️ เส้นทางวันนี้", f"{_today_c}/{DAILY_ROUTE_LIMIT}")
        u2.metric("⚡ โหลดจาก Cache", f"{cache_hits} ครั้ง",
                  delta="เร็วกว่าปกติ" if cache_hits > 0 else None)
