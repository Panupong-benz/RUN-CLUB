...
from db.stats import (init_db, save_profile, get_profile, save_run, get_runs, get_stats,
                      ...
                      save_meetup, get_upcoming_meetups, clear_runs)
...
with tab_plan:
    ...
    with col_map:
        ...
        if st.session_state.last_route and st.session_state.last_lat:
            ...
            # เส้นพื้น (สีทึบ)
            folium.PolyLine(line, color=route_color, weight=6, opacity=0.4).add_to(m)
            # ลูกศรแสดงทิศทางวิ่ง (AntPath)
            AntPath(
                line,
                color=route_color,
                weight=4,
...
4. นัดเจอที่จุดเริ่มต้น แล้ววิ่งด้วยกัน! 🏃‍♂️🏃‍♀️
            ...
            line = [[p[1], p[0]] for p in route.geometry]
            folium.PolyLine(line, color="#16a34a", weight=6, opacity=0.4).add_to(m)
            AntPath(line, color="#16a34a", weight=4, opacity=0.9,
                    delay=600, dash_array=[12, 24], pulse_color="#ffffff").add_to(m)
            if line:
                lats = [p[0] for p in line]
                lons = [p[1] for p in line]
                m.fit_bounds([[min(lats), min(lons)], [max(lats), max(lons)]])
