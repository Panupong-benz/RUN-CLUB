...
def _snap_to_nearest_green(wp_lat: float, wp_lon: float,
    ...
    # Blend 25% toward the green node (ลดจาก 60% เพื่อไม่ให้เบี่ยงออกไกล)
    blend = 0.25
...
def generate_ellipse_waypoints(lat, lon, target_km, num_points=4,
    ...
    if smart:
        # Fetch green/pedestrian nodes within 1× radius
        search_radius_m = int(radius_km * 1000)
        search_radius_m = max(300, min(search_radius_m, 1500))
        green_nodes = _fetch_green_nodes(lat, lon, search_radius_m)

        if green_nodes:
            max_snap = radius_km * 0.25  # snap at most 25% of radius (ลดจาก 50%)
            waypoints = [
                {"lat": snl, "lon": snlo}
                for wp in waypoints
